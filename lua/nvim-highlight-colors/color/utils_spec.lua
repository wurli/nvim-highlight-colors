...
describe("Color Utils", function()
	...
	it("should return hsl color properties", function()
		...
		local hsl_table_decimal = utils.get_hsl_values("hsl(240.5, 80.5%, 68.5%)")
		assert.are.equal(hsl_table_decimal[1], "240.5")
		assert.are.equal(hsl_table_decimal[2], "80.5")
		assert.are.equal(hsl_table_decimal[3], "68.5")
	end)
	...
	it("should return hsl color properties when receiving css variable name", function()
		...
		local hsl_table_decimal = utils.get_hsl_without_func_values("--name: 10.5 20.5% 80.5%;")
		assert.are.equal(hsl_table_decimal[1], "10.5")
		assert.are.equal(hsl_table_decimal[2], "20.5")
		assert.are.equal(hsl_table_decimal[3], "80.5")
	end)
end)

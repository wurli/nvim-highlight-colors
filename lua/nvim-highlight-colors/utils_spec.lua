...
describe("Utils", function()
	...
	it("should create highlight name", function()
		assert.are.equal(utils.create_highlight_name("#FFFFFF"), "nvim-highlight-colors-FFFFFF")
		assert.are.equal(utils.create_highlight_name("0xFFFF00"), "nvim-highlight-colors-0xFFFF00")
		assert.are.equal(utils.create_highlight_name("rgb(255, 255, 255)"), "nvim-highlight-colors-rgb255255255")
		assert.are.equal(utils.create_highlight_name("rgba(255 255 255 0.4)"), "nvim-highlight-colors-rgba25525525504")
		assert.are.equal(utils.create_highlight_name("rgba(0, 255, 0 / 20%)"), "nvim-highlight-colors-rgba0255020")
		assert.are.equal(utils.create_highlight_name("rgba(0, 255, 0 / .2)"), "nvim-highlight-colors-rgba025502")
		assert.are.equal(utils.create_highlight_name("hsl(240, 100%, 68%)"), "nvim-highlight-colors-hsl24010068")
		assert.are.equal(utils.create_highlight_name("hsl(150deg 30% 40%) "), "nvim-highlight-colors-hsl150deg3040")
		assert.are.equal(utils.create_highlight_name("hsl(0.3turn 60% 15%)"), "nvim-highlight-colors-hsl03turn6015")
		assert.are.equal(utils.create_highlight_name("bg-sky-50"), "nvim-highlight-colors-bgsky50")
		assert.are.equal(utils.create_highlight_name("var(--testing-color)"), "nvim-highlight-colors-vartestingcolor")
		assert.are.equal(utils.create_highlight_name("\\033[0;30m"), "nvim-highlight-colors-030m")
		assert.are.equal(utils.create_highlight_name("blue"), "nvim-highlight-colors-blue")
	end)
	...
	it("should create highlight for hex color in foreground mode", function()
		...
		utils.create_highlight(params.buffer_id, params.ns_id, params.data, params.options)
		assert
			.spy(vim.api.nvim_set_hl).was
			.called_with(0, "nvim-highlight-colors-foregroundFFFFFFFFFFFF", { fg = "#FFFFFF", default = true })
	...
	it("should create highlight for hex color in background mode", function()
		...
		assert
			.spy(vim.api.nvim_set_hl).was
			.called_with(0, "nvim-highlight-colors-backgroundFFFFFFFFFFFF", { fg = "#000000", bg = "#FFFFFF", default = true })
		assert.spy(vim.api.nvim_buf_add_highlight).was.called_with(
			params.buffer_id,
			params.ns_id,
	...
	it("should create highlight for short hex color in background mode", function()
		...
		assert
			.spy(vim.api.nvim_set_hl).was
			.called_with(0, "nvim-highlight-colors-backgroundFFFFFFFFF", { fg = "#000000", bg = "#FFFFFF", default = true })
		assert.spy(vim.api.nvim_buf_add_highlight).was.called_with(
			params.buffer_id,
			params.ns_id,
	...
	it("should create highlight for custom colors in background mode", function()
		...
		assert
			.spy(vim.api.nvim_set_hl).was
			.called_with(0, "nvim-highlight-colors-backgroundcustomcolorFFFFFF", { fg = "#000000", bg = "#FFFFFF", default = true })
	...
	it("should create highlight for hex colors in virtual mode", function()
		...
		assert
			.spy(vim.api.nvim_buf_set_extmark).was
			.called_with(params.buffer_id, params.ns_id, params.data.row + 1, params.data.start_column, {
				hl_mode = "combine",
				virt_text = {
					{
						"_■=",
	...
	it("should replace highlight for hex colors in virtual mode", function()
		...
		utils.highlight_extmarks(params.buffer_id, params.ns_id, params.data, params.highlight_group, params.options)

		assert
			.spy(vim.api.nvim_buf_del_extmark).was
			.called_with(params.buffer_id, params.ns_id, already_highlighted_extmark_id)
	...
	it("should return virtual symbol position if neovim version if higher then 0.9", function()
		...
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "eow" }), "eow")
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "eol" }), "eol")
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "inline" }), "inline")
	end)
	...
	it('should return "eol" as virtual symbol position if neovim version if 0.9 or below', function()
		...
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "eow" }), "eol")
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "eol" }), "eol")
		assert.are.equal(utils.get_virtual_text_position({ virtual_symbol_position = "inline" }), "eol")
	end)
	...
	it("should call highlight_lsp_document_color only if supported LSP is detected", function()
		...
		utils.highlight_with_lsp(params.buffer_id, params.ns_id, params.data, params.options)

		assert.spy(utils.highlight_lsp_document_color).was_called(2)
		assert
			.spy(utils.highlight_lsp_document_color)
			.was_called_with(lsp_response, params.buffer_id, params.ns_id, params.data, params.options)
	end)
end)
